...
export const createChannel = async (request, response) => {
  try {
    ...
    if (!admin) return response.status(400).send("Admin user not found.");
...
export const getChannelMessages = async (request, response, next) => {
  try {
    ...
    // FIX: Add a check to ensure channelId is a valid ObjectId
    if (!mongoose.Types.ObjectId.isValid(channelId)) {
        console.log(`SERVER: Invalid channelId provided: ${channelId}`);
